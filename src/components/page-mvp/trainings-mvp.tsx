 ...
 * Lead sekcji tłumaczy formę: nagrany kurs online (moduły + lekcje),
 * test 10–20 pytań, imienny certyfikat — stacjonarnie na życzenie.
 * Warsztat „przy kawie” celowo NIE jest tu wliczany — ma osobną sekcję.
 */
export function TrainingsMvp() {
  const [value, setValue] = React.useState<string>(trainingsF[0].number);
  const reduce = useReducedMotion();

  return (
    <SectionMvp id="oferta">
      <ContainerMvp>
        <SectionHeadMvp
            ...
            {trainingsF.map((training) => {
              ...
              return (
                ...
                >
                  {isActive ? (
                    <motion.span
                      layoutId={reduce ? undefined : "training-surface-mvp"}
                      aria-hidden
...
function TrainingPanel({
  ...
  return (
    ...
    >
      ...
      {rest.length > 0 ? (
        <div
          className={`${T_BODY_MVP} text-muted-foreground mt-5 gap-10 space-y-4 text-pretty md:columns-2 md:space-y-0 [&>p]:break-inside-avoid md:[&>p+p]:mt-0`}
        >
          {rest.map((paragraph) => (
