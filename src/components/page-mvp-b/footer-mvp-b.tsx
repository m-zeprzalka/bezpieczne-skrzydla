...
import { CONTAINER_MVPB, T_LABEL_MVPB } from "@/components/page-mvp-b/frame-mvp-b";
import { WingRule } from "@/components/site/wing-arcs";
import { site } from "@/lib/content";
import { footerMvp, navMvp } from "@/lib/content-mvp";

/**
 * Stopka z korektą z maila: bez frazy „szkolenia online i stacjonarne”,
 * która czytała się jak obietnica zajęć na żywo — opis mówi wprost
 * o nagranych kursach i stacjonarnych szkoleniach na życzenie.
 */
export function FooterMvpB() {
  const year = new Date().getFullYear();

  return (
    <footer className="border-brand-100 border-t bg-white">
      <div className={`${CONTAINER_MVPB} py-20 md:py-24`}>
        ...
        <div className="grid grid-cols-1 gap-x-10 gap-y-14 md:grid-cols-12">
          <div className="md:col-span-6">
            ...
            <p className="text-muted-foreground mt-7 max-w-[26rem] text-[0.9375rem] leading-[1.8]">
              {footerMvp.description}
            </p>
          </div>

          <nav aria-label="Nawigacja w stopce" className="md:col-span-3">
            <p className={`${T_LABEL_MVPB} text-brand-600`}>Strona</p>
          ...
          <div className="md:col-span-3">
            <p className={`${T_LABEL_MVPB} text-brand-600`}>Kontakt</p>
            ...
            <dl className="text-muted-foreground mt-6 flex flex-col gap-1 text-[0.8125rem]">
        ...
        <div className="border-brand-100 text-muted-foreground mt-16 flex flex-col gap-4 border-t pt-9 text-[0.8125rem] sm:flex-row sm:items-center sm:justify-between">
          <p>
