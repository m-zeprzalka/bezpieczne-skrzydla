 ...
 * Tło: sama poświata — bez siatek i ornamentów. Ton „warm” tylko dla warsztatu.
 */
export function PageHero({
  ...
  return (
    <header className={cn("relative overflow-hidden", warm ? "bg-sand-50" : "bg-white", className)}>
      <div
        aria-hidden
        className={cn(
      ...
      >
        <div className={cn(aside ? "lg:col-span-7" : "lg:col-span-8")}>
          ...
          {label ? (
            <Reveal delay={0.04} as="p" className="t-label mt-10 text-brand-600">
              {label}
            </Reveal>
          ) : null}

          <Reveal delay={0.08} as="div" className="mt-5">
            <h1 className="text-h1 max-w-[16ch] text-balance text-ink">
              {title}
              {accent ? (
                <>
                  {" "}
          ...
          {lead ? (
            <Reveal
              delay={0.14}
              as="p"
              className="text-lead mt-6 max-w-[34rem] text-pretty text-ink-muted"
            >
              {lead}
            </Reveal>
          ) : null}
