...
import { ContainerF, T_LEAD_F } from "@/components/page-f/frame-f";
import {
  Reveal,
  RevealGroup,
  RevealItem,
...
import { heroChipsG, heroStatsG, marqueeG } from "@/lib/content-g";
...
/**
 * Hero G: te same teksty klientki co w F, ale w kompozycji dzielonej —
 * po prawej kadr na zdjęcie autorki z pływającymi plakietkami, pod spodem
 * pas czterech liczb i przewijany pasek haseł.
 */
export function HeroG() {
  return (
    <section className="relative overflow-hidden bg-white">
      ...
      <ContainerF className="grid grid-cols-1 items-center gap-x-8 gap-y-16 pt-16 pb-20 sm:pt-20 md:grid-cols-12 lg:pt-24 lg:pb-24">
        ...
        <div className="md:col-span-7 lg:pr-6">
          ...
          <Reveal
            delay={0.55}
            className="font-display text-brand-700 mt-7 max-w-[34rem] text-[1.2rem] leading-[1.45] italic sm:text-[1.35rem]"
          >
            {heroF.lead}
          </Reveal>
          ...
          <Reveal
            delay={0.65}
            className={`${T_LEAD_F} text-muted-foreground mt-5 max-w-[36rem] text-pretty`}
          >
            {heroF.body}
          </Reveal>
      ...
      <div className="border-brand-100 relative overflow-hidden border-y bg-white py-4">
        <div
          aria-hidden
          className="mask-fade-edges animate-marquee flex w-max items-center gap-9 motion-reduce:animate-none"
        >
          {[...marqueeG, ...marqueeG].map((item, i) => (
            <span key={`${item}-${i}`} className="flex items-center gap-9">
              <span className="text-brand-700/85 text-[0.85rem] font-medium tracking-wide whitespace-nowrap">
                {item}
              </span>
              <span
                aria-hidden
                className="bg-brand-300 size-1.5 shrink-0 rounded-full"
              />
            </span>
          ))}
        </div>
