      ...
      <RevealGroup className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        {plans.map((plan) => (
          <RevealItem key={plan.id} className="flex">
            <article
              id={plan.id}
              className="flex w-full scroll-mt-28 flex-col rounded-card border border-white/10 bg-white/[0.05] p-7 transition-colors duration-300 hover:bg-white/[0.08] lg:p-8"
            >
              <span className="t-label text-brand-300">{plan.name}</span>
              <p className="mt-6 font-display text-[1.75rem] leading-none tracking-tight text-white">{plan.price}</p>
              <p className="mt-1.5 text-caption text-brand-300/80">{plan.audience}</p>
              <p className="mt-6 flex-1 text-small text-pretty text-brand-100/85">{plan.description}</p>
              <Button asChild variant="inverse" size="lg" className="mt-8 w-full">
