  ...
  return (
    <Reveal delay={0.08} className={cn("", className)}>
      <Accordion
        ...
        className="w-full overflow-hidden rounded-panel border border-brand-200 bg-white shadow-lift"
      >
        {items.map((item) => (
          <AccordionItem key={item.id} value={item.id} className="border-brand-100 px-6 sm:px-8">
            <AccordionTrigger className="gap-6 py-6 text-left font-display text-[1.05rem] leading-snug font-medium text-ink hover:no-underline data-[state=open]:text-brand-700 sm:text-[1.15rem] [&_svg]:text-brand-500">
              {item.q}
            </AccordionTrigger>
